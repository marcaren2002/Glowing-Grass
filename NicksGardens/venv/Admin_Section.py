...
pygame.display.set_caption("Administrative")
...
def run_Admin_Window():
    def login():
        ...
        while admin_verified == False:
            if admin_username_input.text == admin_username and admin_password_input.text == admin_password:
                print("Welcome Nicholas!")
                break
            else:
                print("Incorrect login details")
                break
    admin_username = "NicholasGrass88"
    admin_password = "12GardenAdmin"

    adminWindow_title = textBox(100,50,0,0,"Administrative Section")

    admin_username_input = Input_textBox(200,150,140,32,"")
    admin_password_input = Input_textBox(200,200,140,32,"")

    admin_login = Button("Log In",15,200,250,50,50,red,green,login)
    proceed = Button("Proceed",15,200,300,50,40,red,green,)

    buttons = [admin_login]
    labels = [adminWindow_title]
    input_boxes = [admin_username_input,admin_password_input]
    done = False
